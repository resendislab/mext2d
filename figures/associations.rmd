...
```{r}
data[, .(pval = kruskal.test(reads ~ value)$p.value), by=taxa]
```
...
ggplot(sdata, aes(x=diabetes_status, y=beta_cell_disposition_index)) + geom_boxplot() + geom_jitter(width=0.25, alpha=0.25)
sdata$beta_cells_affected <- as.integer(sdata$beta_cell_disposition_index < 1)
...
```{r, fig.width=6, fig.height=6}
genera <- c("Escherichia/Shigella", "Anaerostipes", "Blautia", "Veillonella")
plot_counts(naive, "beta_cells_affected", "genus", genera, normalized = T) +
