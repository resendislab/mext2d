...
```{r}
filtered[, padj := IHW::adj_pvalues(IHW::ihw(pvalue ~ baseMean, alpha=0.1))]
filtered[padj < 0.05 & genus %in% genera]
```

Thus, the only association that holds up significant after confounding is the connection between Escherichia and 
...
Genera which are associated significantly with richness (Shannon index). Error bars denotes the standard error of the log2 fold change (regression coefficient).
